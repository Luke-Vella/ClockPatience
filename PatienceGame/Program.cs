...
namespace ClockPatience.ConsoleApp
{
    public class Program
    {
        ...
        {
            // Firstly, we're going to ask the user for his/her prefered language, because over-engineering a simple console app is fun.
        ...
        private static void PlayGame(ILocalizationProvider loc)
        {
            ...
            // Instantiate game service and start a new game
            IClockPatienceGameService gameService = new ClockPatienceGameService();
            gameService.StartNewGame();

            PrintSeperatorInTerminal(false, true);
            Console.WriteLine(loc.Get("GAME_START_MESSAGE"));

            // Seed the game with the desired amount of decks from the user's input
            List<DeckDTO> seededDecks = gameService.SeedDecks(PromptUserToSeedDecks(loc));
        ...
        {
            PrintSeperatorInTerminal(true, true);
            Console.WriteLine(loc.Get("WELCOME_MESSAGE_LN_1"));
            PrintSeperatorInTerminal(false,false,true);
        ...
        private static void PrintSeperatorInTerminal(bool includeLineSeperator = true, bool includeEmptyLineBefore = false, bool includeEmptyLineAfter = false)
        {
